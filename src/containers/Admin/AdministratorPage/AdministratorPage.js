...
class AdministratorPage extends Component {
  ...
  handleSelectedPerson = event => {
    if (event.currentTarget.id === "Student") {
      ...
      this.props.history.push("/studentregis");
    } else if (event.currentTarget.id === "Teacher") {
    ...
    return (
      <div className={styles.container}>
        <div className={styles.admin_controller_card}>
          <button className={styles.admin_controller_card_button}>
            Registration
          </button>
          <button
          ...
          >
            <button id="Student" onClick={this.handleSelectedPerson}>
              Student
            </button>
            <button id="Teacher" onClick={this.handleSelectedPerson}>
              Teacher
            </button>
        ...
        <div className={styles.admin_controller_card}>
          ...
          <button
            className={styles.dropDown_button}
            onClick={this.handleOpenAddNewDropDown}
          >
            <span>{this.state.selectedAddNew}</span>
            <div>
              {this.state.isOpenAddNewDropDown ? (
                <IoMdArrowDropup size="1.5em" />
              ) : (
                <IoMdArrowDropdown size="1.5em" />
              )}
          ...
          >
            <button id="Class" onClick={this.handleSelectedAddNew}>
              Class
            </button>
            <button id="Subject" onClick={this.handleSelectedAddNew}>
              Subject
            </button>
          </div>
        </div>
        ...
        {this.state.isOpenAddClassModal && (
          <AddClassModal closeModal={this.handleCloseAddClassModal} />
        )}
        {this.state.isOpenAddSubjectModal && (
          <AddSubjectModal closeModal={this.handleCloseAddSubjectModal} />
        )}
        {this.state.isOpenAssignClassModal && (
          <AssignClassModal closeModal={this.handleCloseAssignClassModal} />
        )}
      </div>
    );
  }
